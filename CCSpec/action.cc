...
/* CLP ADDED
 * New Constructor to set the LP operation
 *
 */
ModelAction::ModelAction(action_type_t type, action_type_t op, std::string func_pair,     
		void *loc, memory_order order, uint64_t value, Thread *thread) :
	type(type),
	order(order),
	location(loc),
	next_op(op), //CLP ADDED
	//function(""), //CLP ADDED
	function_pair(func_pair),
	value(value),
	reads_from(NULL),
	reads_from_promise(NULL),
	last_fence_release(NULL),
...
/* CLP ADDED
 * New Constructor to identify the result
 *
 */
ModelAction::ModelAction(action_type_t type, action_type_t op,     
		void *loc, memory_order order, uint64_t value, Thread *thread) :
	type(type),
	order(order),
	location(loc),
...
/* CLP ADDED
 * New Constructor to identify the invariant, correctness condition, invocation, or response
 *
 */

ModelAction::ModelAction(action_type_t type, std::string func, memory_order order, 
	    uint64_t value, Thread *thread) :
	type(type),
	order(order),
	location(NULL),
	function(func), //CLP ADDED
	value(value),
	reads_from(NULL),
	reads_from_promise(NULL),
	last_fence_release(NULL),
...
/* CLP ADDED
 * New Constructor to identify the invocation/response with input/output
 *
 */
ModelAction::ModelAction(action_type_t type, std::string func, uint64_t io, void *loc, memory_order order, 
	    uint64_t value, Thread *thread) :
	type(type),
	order(order),
	location(loc),
	function(func), //CLP ADDED
	input_output(io), //CLP ADDED
	input_output2(0), //CLP ADDED
	value(value),
	reads_from(NULL),
	reads_from_promise(NULL),
	last_fence_release(NULL),
 ...
 *
 */
ModelAction::ModelAction(action_type_t type, std::string func, uint64_t io, uint64_t io2, void *loc, memory_order order, 
	    uint64_t value, Thread *thread) :
	type(type),
	order(order),
	location(loc),
...
/* CLP ADDED
 * New Constructor to set the Variable and Validation
 *
 */
ModelAction::ModelAction(action_type_t type, action_type_t op, std::string variable,     
		std::string validation, memory_order order, uint64_t value, Thread *thread) :
	type(type),
	order(order),
	location(NULL),
	next_op(op), //CLP ADDED
	//function(""), //CLP ADDED
	variable(variable),
	validation(validation), //CLP ADDED
	value(value),
	reads_from(NULL),
	reads_from_promise(NULL),
	last_fence_release(NULL),
...
/* CLP ADDED
 * New Constructor to create an Action Object used by the parser
 *
 */
ModelAction::ModelAction(action_type_t type, memory_order order, thread_id_t tid, modelclock_t seq_number, 
        uint64_t value) :
	type(type),
	order(order),
	location(NULL),
...
//CLP ADDED
bool ModelAction::is_linearization_point() const
{
	return type == LINEARIZATION_POINT;
}
...
/* CLP ADDED
 * Change memory model order for action
 *
 */
void ModelAction::set_mo(memory_order order)
{
    this->order = order;
    return;
}

void ModelAction::set_LP()
{
    this->LP = true;
}

bool ModelAction::get_LP()
{
    return this->LP;
}

void ModelAction::set_result()
{
    this->result = true;
}

bool ModelAction::get_result()
{
    return this->result;
}


action_type_t ModelAction::get_next_op()
{
    return next_op;
}

std::string * ModelAction::get_function()
{
    //model_print("Function = %s\n", function);
    return &(this->function);
}

std::string * ModelAction::get_function_pair()
{
    //model_print("Function = %s\n", function);
    return &(this->function_pair);
}

std::string * ModelAction::get_variable()
{
    //model_print("Function = %s\n", function);
    return &(this->variable);
}

std::string * ModelAction::get_validation()
{
    //model_print("Function = %s\n", function);
    return &(this->validation);
}

uint64_t ModelAction::get_input_output()
{
    return this->input_output;
}

uint64_t ModelAction::get_input_output2()
{
    return this->input_output2;
}
