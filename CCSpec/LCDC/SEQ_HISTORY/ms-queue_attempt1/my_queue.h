...
typedef atomic<unsigned long> pointer_t;
...
static unsigned int new_node()
{
	...
	for (i = 0; i < MAX_FREELIST; i++) {
		...
		unsigned int node =	*((uint32_t *) &free_lists[t][i]);
		if (node) {
			//store_32(&free_lists[t][i], 0);
			(*(uint32_t *) &free_lists[t][i]) = 0;
			return node;
		}
	}
	/* free_list is empty? */
...
static void reclaim(unsigned int node)
{
	...
	for (i = 0; i < MAX_FREELIST; i++) {
		...
		unsigned int idx = *((uint32_t *) &free_lists[t][i]);

		/* Found empty spot in free list */
		if (idx == 0) {
			//store_32(&free_lists[t][i], node);
			(*(uint32_t *) &free_lists[t][i]) = node;
			return;
		}
	}
	/* free list is full? */
...
void init_queue(queue_t *q, int num_threads)
{
	...
	for (i = 0; i < num_threads; i++) {
		for (j = 0; j < INITIAL_FREE; j++) {
			...
			q->nodes[free_lists[i][j]].next.store(MAKE_POINTER(POISON_IDX, 0), memory_order_relaxed);
		}
	}

	/* initialize queue */
	//atomic_init(&q->head, MAKE_POINTER(1, 0));
	q->head.store(MAKE_POINTER(1, 0), memory_order_relaxed);
	//atomic_init(&q->tail, MAKE_POINTER(1, 0));
	q->tail.store(MAKE_POINTER(1, 0), memory_order_relaxed);
	//atomic_init(&q->nodes[1].next, MAKE_POINTER(0, 0));
	q->nodes[1].next.store(MAKE_POINTER(0, 0), memory_order_relaxed);
}
...
void enqueue(queue_t *q, unsigned int val)
{
	...
	(*(uint32_t *)&q->nodes[node].value) = val;
	...
	while (!success) {
		...
		if (atomic_load_explicit(&q->head, relaxed) == head) {
			if (get_ptr(head) == get_ptr(tail)) {
				...
				*retVal = *((uint32_t *) &q->nodes[get_ptr(next)].value);
