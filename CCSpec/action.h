...
public:
	...
	//CLP ADDED
	ModelAction(action_type_t type, action_type_t op, std::string func_pair, void *loc, memory_order order = memory_order_seq_cst, uint64_t value = VALUE_NONE, Thread *thread = NULL);
	//CLP ADDED
	ModelAction(action_type_t type, action_type_t op, void *loc, memory_order order = memory_order_seq_cst, 
	    uint64_t value = VALUE_NONE, Thread *thread = NULL);
	//CLP ADDED
	ModelAction(action_type_t type, std::string func, memory_order order = memory_order_seq_cst, 
	    uint64_t value = VALUE_NONE, Thread *thread = NULL);	
	//CLP ADDED
	ModelAction(action_type_t type, std::string func, uint64_t io, void *loc = NULL, memory_order order = memory_order_seq_cst, 
	    uint64_t value = VALUE_NONE, Thread *thread = NULL);
	//CLP ADDED
	ModelAction(action_type_t type, std::string func, uint64_t io, uint64_t io2, void *loc = NULL, memory_order order = memory_order_seq_cst, 
	    uint64_t value = VALUE_NONE, Thread *thread = NULL);		
	//CLP ADDED
	ModelAction(action_type_t type, action_type_t op, std::string variable, std::string validation, memory_order order = memory_order_seq_cst, uint64_t value = VALUE_NONE, Thread *thread = NULL);
	//CLP ADDED
	ModelAction(action_type_t type, memory_order order, thread_id_t tid, modelclock_t seq_number, uint64_t value = VALUE_NONE);
	~ModelAction();
	void print() const;
	...
	bool is_linearization_point() const; //CLP ADDED
	...
	//CLP ADDED
	//const act may cause this method problems
	void set_mo(memory_order order);
	...
	void set_result();
	bool get_result();
	action_type_t get_next_op();
	std::string * get_function();
	std::string * get_function_pair();
	std::string * get_variable();
	std::string * get_validation();	
	uint64_t get_input_output();
	uint64_t get_input_output2();
