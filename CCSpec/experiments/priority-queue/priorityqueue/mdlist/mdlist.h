...
public:
    ...
    struct Node
    {
		...
		std::atomic<Node *> m_child[DIMENSION];
        uint32_t m_key;
        uint8_t m_coord[DIMENSION];
        uint32_t m_seq;
        /*Node* m_purged;*/
		std::atomic<Node*> m_purged;
        /*Desc* m_pending;*/            //pending operation to update the children of 
		std::atomic<Desc*> m_pending;
		
        Node() 
        {
			for(int i = 0; i < DIMENSION; i++)
			{
				m_child[i].store(NULL);
			}
			m_purged.store(NULL);
			m_pending.store(NULL);
        }
    };
...
private:
    ...
    /*Stack* m_stack;*/
	std::atomic<Stack*> m_stack;
    char m_pad1[CACHE_LINE_SIZE - sizeof(Node*)];
    /*Stack* m_purge;*/
	std::atomic<Stack*> m_purge;
};
