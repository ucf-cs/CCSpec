...
{
	QueueAdaptor<MDList> *pq = (QueueAdaptor<MDList> *) arg;
	pq->Insert(300);
	pq->Insert(100);
	pq->DeleteMin();	
}
...
int user_main(int argc, char **argv)
{
	...
	*/
	QueueAdaptor<MDList> pq(testSize, numThread + 1);
	//Tester(numThread, testSize, portion, pq);

	pq.Init();

	thrd_create(&t1, ThreadA, &pq);
	thrd_create(&t2, ThreadB, &pq);
